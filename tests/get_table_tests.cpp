...
BOOST_FIXTURE_TEST_CASE( get_scope_test, TESTER ) try {
   ...
   create_accounts({ N(lemon.token), N(lemon.ram), N(lemon.ramfee), N(lemon.stake),
      N(lemon.bpay), N(lemon.vpay), N(lemon.saving), N(lemon.names) });

   std::vector<account_name> accs{N(inita), N(initb), N(initc), N(initd)};
   create_accounts(accs);
   produce_block();

   set_code( N(lemon.token), contracts::eosio_token_wasm() );
   set_abi( N(lemon.token), contracts::eosio_token_abi().data() );
   produce_blocks(1);
   ...
   push_action(N(lemon.token), N(create), N(lemon.token), act );

   // issue
   for (account_name a: accs) {
      push_action( N(lemon.token), N(issue), "lemon", mutable_variant_object()
                  ("to",      name(a) )
   ...
   eosio::chain_apis::read_only::get_table_by_scope_params param{N(lemon.token), N(accounts), "inita", "", 10};
   ...
   if (result.rows.size() >= 4) {
      BOOST_REQUIRE_EQUAL(name(N(lemon.token)), result.rows[0].code);
      BOOST_REQUIRE_EQUAL(name(N(inita)), result.rows[0].scope);
      BOOST_REQUIRE_EQUAL(name(N(accounts)), result.rows[0].table);
      BOOST_REQUIRE_EQUAL(name(N(lemon)), result.rows[0].payer);
...
BOOST_FIXTURE_TEST_CASE( get_table_test, TESTER ) try {
   ...
   if (result.rows.size() >= 4) {
      ...
      BOOST_REQUIRE_EQUAL("lemon", result.rows[0]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("lemon", result.rows[1]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("lemon", result.rows[2]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("lemon", result.rows[3]["payer"].as_string());
   }
   p.show_payer = false;
...
BOOST_FIXTURE_TEST_CASE( get_table_by_seckey_test, TESTER ) try {
   ...
   auto bidname = [this]( const account_name& bidder, const account_name& newname, const asset& bid ) {
      return push_action( N(lemon), N(bidname), bidder, fc::mutable_variant_object()
                          ("bidder",  bidder)
                          ("newname", newname)
                          ("bid", bid)
                          );
