...
const static uint64_t system_account_name    = N(lemonc);
const static uint64_t null_account_name      = N(lemonc.null);
const static uint64_t producers_account_name = N(lemonc.prods);
...
const static uint64_t eosio_auth_scope       = N(lemonc.auth);
const static uint64_t eosio_all_scope        = N(lemonc.all);

const static uint64_t active_name = N(active);
const static uint64_t owner_name  = N(owner);
const static uint64_t eosio_any_name = N(lemonc.any);
const static uint64_t eosio_code_name = N(lemonc.code);
