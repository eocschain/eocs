...
string default_wallet_url = "unix://" + (determine_home_directory() / "lemon-wallet" / (string(key_store_executable_name) + ".sock")).string();
...
fc::variant regproducer_variant(const account_name& producer, const public_key_type& key, const string& url, uint16_t location, const account_name& regaccount) {
...
inline asset to_asset( const string& s ) {
   return to_asset( N(lemon.token), s );
}
...
struct register_producer_subcommand {
   ...
   register_producer_subcommand(CLI::App* actionRoot) {
      ...
      register_producer->set_callback([this] {
         ...
         auto regprod_var = regproducer_variant(producer_str, producer_key, url, loc , regaccount_str);
         auto accountPermissions = get_account_permissions(tx_permission, {regaccount_str,config::active_name});
   ...
   bidname_info_subcommand(CLI::App* actionRoot) {
      ...
      list_producers->set_callback([this] {
         auto rawResult = call(get_table_func, fc::mutable_variant_object("json", true)
                               ("code", "lemon")("scope", "lemon")("table", "namebids")
...
//                                ->check(CLI::ExistingFile);
   ...
   propose_action->set_callback([&] {
      ...
      send_actions({chain::action{accountPermissions, "lemon.msig", "propose", variant_to_bin( N(lemon.msig), N(propose), args ) }});
   });
   ...
   auto approve_or_unapprove = [&](const string& action) {
      ...
      send_actions({chain::action{accountPermissions, "lemon.msig", action, variant_to_bin( N(lemon.msig), action, args ) }});
   };
   ...
   invalidate->set_callback([&] {
      ...
      send_actions({chain::action{accountPermissions, "lemon.msig", "invalidate", variant_to_bin( N(lemon.msig), "invalidate", args ) }});
   });
   ...
   cancel->set_callback([&]() {
      ...
      send_actions({chain::action{accountPermissions, "lemon.msig", "cancel", variant_to_bin( N(lemon.msig), N(cancel), args ) }});
      }
   );
   ...
   exec->set_callback([&] {
      ...
      send_actions({chain::action{accountPermissions, "lemon.msig", "exec", variant_to_bin( N(lemon.msig), N(exec), args ) }});
      }
   );
